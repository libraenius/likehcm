        ...
        {isQuarterlyExpanded && (
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2 mt-3 mb-3">
              ...
              <div className="flex items-center gap-2">
                <Label htmlFor="edit-mode-quarterly-toggle" className="text-sm font-medium cursor-pointer flex items-center gap-2">
                  <Edit className="h-4 w-4" />
                  Режим редактирования
                </Label>
                <Switch
                  id="edit-mode-quarterly-toggle"
                  checked={Object.values(isEditModeQuarterly).some(v => v)}
                  onCheckedChange={(checked) => {
                    const newModes: Record<string, boolean> = {};
                    ["q1", "q2", "q3", "q4"].forEach(q => {
                      newModes[`${q}-${selectedQuarterlyYear}`] = checked;
                    });
